...
export const listEmployeesByCompany = async (req, res, next) => {
  try {
    res.json(await employeeService.listByCompany(req.params.companyId));
  } catch (err) {
    next(err);
  }
};
...
export const getMyEmployee = async (req, res, next) => {
  try {
    res.json(await employeeService.getByUser(req.user.id));
  } catch (err) {
    next(err);
  }
};
